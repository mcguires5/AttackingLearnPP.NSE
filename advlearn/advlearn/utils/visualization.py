...
class PoisonViz(object):
    def __init__(self, attack_class, args, kwargs, data_training, labels_training,
                 data_validation, labels_validation, verbose=2):
    ...
    def __get_loss(self, xx, yy, xx_ravel, yy_ravel, target):
        ...
        for ind, (x, y) in enumerate(zip(xx_ravel, yy_ravel)):
            z_objective[ind] = attack.attack_loss(
    ...
    def plot_loss(self, subplot, mesh_x_steps, mesh_y_steps, gradient=True, decision=True, target=1):
        ...
        # Plot the Data
        ax.scatter(self.data_training[self.labels_training == -1, 0],
                   self.data_training[self.labels_training == -1, 1],
                   c='b', edgecolors='k', zorder=10)
        ax.scatter(self.data_training[self.labels_training == 1, 0],
                   self.data_training[self.labels_training == 1, 1],
                   c='r', edgecolors='k', zorder=10)
        ...
        if decision:
            ...
            ax.contour(xx, yy, z_decision, colors='k', levels=[-1, 0, 1], alpha=0.5,
                       linestyles=['--', '-', '--'])
            ...
            margin_sv_ind = np.argwhere(np.logical_and(alpha > 0, alpha < self.kwargs['c']))
            ...
            ax.scatter(self.data_training[margin_sv_ind, 0], self.data_training[margin_sv_ind, 1], s=100, facecolors='none',
                       edgecolors='red', zorder=15)
            ax.scatter(self.data_training[error_sv_ind, 0], self.data_training[error_sv_ind, 1], s=100, facecolors='none',
                       edgecolors='pink', zorder=15)
            #ax.scatter(self.data[reserve_sv_ind, 0], self.data[reserve_sv_ind, 1], s=100, facecolors='none',
            #           edgecolors='purple', zorder=15)

        ax.set_autoscale_on(True)
        self.figure.colorbar(cs_minor, ax=ax, orientation='horizontal', format='%0.3f',
                             pad=0.04)
