...
import numpy as np
from advlearn.attacks.dummy import DummyEvadeAttack, DummyPoisonAttack
...
class TestDummyPoisonAttack(object):
    """Test dummy poisoning attack"""
    def test_attack_point(self):
        """Test that the attack point is a numpy array of the correct size.
        """
        model = DummyPoisonAttack()
        model.fit(X, Y)
        (x_attack, y_attack) = model.get_attack_point()
        assert isinstance(x_attack, np.ndarray)
        assert x_attack.shape == (1, 2)


class TestDummyEvadeAttack(object):
    """Test dummy poisoning attack"""
    def test_attack_point(self):
        """Test that the attack point is a numpy array of the correct size.
        """
        model = DummyEvadeAttack()
        model.fit(X, Y)
        (x_attack, y_attack) = model.get_attack_point()
        assert isinstance(x_attack, np.ndarray)
        assert x_attack.shape == (1, 2)
