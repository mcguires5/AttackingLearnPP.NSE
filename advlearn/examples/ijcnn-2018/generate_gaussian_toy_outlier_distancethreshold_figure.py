...
def create_toy_data(random_state=None):
    centers = [[-1, 0], [1, 0]]
    X, y = make_blobs(n_samples=50, centers=centers, cluster_std=0.5,
...
def get_gradient(attack=None):
    ...
    for j, _ in enumerate(xx_ravel):
        z_gradient[j, :] = attack.attacker_objective_direction(
...
def get_objective(attack=None):
    ...
    for j, _ in enumerate(xx_ravel):
        z_objective[j] = attack.mse_error(
            np.array([xx_ravel[j], yy_ravel[j]]), np.array([1]))
...
def main():
    ...
    x_attack_trajectory, x_attack, y_attack = get_trajectory(attack)
    ...
    ax.set_title("Objective Function - New")
    ...
    #############################
    print('Finding classification error...')
